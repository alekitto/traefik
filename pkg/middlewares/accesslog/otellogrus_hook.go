...
// Package accesslog provides a [Hook], a [logrus.Hook] implementation that
...
func (h *Hook) convertEntry(e *logrus.Entry) log.Record {
	...
	formatted, _ := h.formatter.Format(e)
	kvs := append(convertFields(e.Data), log.KeyValue{
		Key:   "_msg",
		Value: ConvertValue(string(formatted[:])),
	})
