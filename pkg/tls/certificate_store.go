...
func (c *CertificateStore) GetBestCertificate(clientHello *tls.ClientHelloInfo) *TLSCertificate {
	if c == nil {
		return nil
	}
	...
	matchedCerts := map[string]*TLSCertificate{}
	if c.DynamicCerts != nil && c.DynamicCerts.Get() != nil {
		for domains, cert := range c.DynamicCerts.Get().(map[string]*TLSCertificate) {
